...
void transmitDataString(char data_mode, char addr[4],char rssi[3], char msg[MESSAGE_LENGTH] )
{
  ...
  {
    char output_short[] = {"RSI"};

    output_short[0] = rssi[2];
    output_short[1] = rssi[1];
    output_short[2] = rssi[0];
    TXString(output_short, sizeof output_short );
  }
}

void transmitData(int addr, signed char rssi,  char msg[MESSAGE_LENGTH], char *ret)
{
  char addrString[4];
  char rssiString[3];
  ...
  *ret++ =  rssiString[1];
  *ret++ =  rssiString[2];
  //ansmitDataString( degCMode, addrString, rssiString, msg );
}
