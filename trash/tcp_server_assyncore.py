...
#Execute polling in mysql
def mysqlpoll():
    while True:
        print "The time is: "+strftime("%Y-%m-%d %H:%M:%S", gmtime())
        time.sleep(1)
        if sensors_ip[2] != 0:
            Send_Client(socket_dev[2],"Hello World")
...
thread1 = Thread(1, "socket", 1)
thread2 = Thread(2, "MySQLpoll", 2)
#
thread1.daemon = True #Guarantess the dead threads when we want
thread2.daemon = True #Guarantess the dead threads when we want
thread1.start()
thread2.start()
