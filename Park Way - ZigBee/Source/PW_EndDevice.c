...
#define   DISTANCE_CONSTANT          40000     //Distance measured by the ultrasonic
#define   PIN_GREEN_LED               0x08     //P0.3
#define   PIN_RED_LED                 0x10     //P0.4
#define   TIME_ULTRA                   200     //Poll time to read ultrasonic
#define   DEBUG_ULTRA                          //Used to send ultrasound read to the AP
/*********************************************************************
 * TYPEDEFS
 */
...
{
    uint16 i,timeout_1=0,timeout_2=0;
    ...
    if(!(P0&PIN_ECHO))
    {
      ...
      Onboard_wait(6); 
      //&&(timeout_1<40000)
      while(!(P0&PIN_ECHO)&&(timeout_2<100))
      {
        if(timeout_1<65000)
        { 
          timeout_1++;
